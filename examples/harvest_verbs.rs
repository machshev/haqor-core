...
fn stem_name(c: char) -> Option<&'static str> {
    Some(match c {
        'o' => "Polel", 'O' => "Polal", 'r' => "Hithpolel", 'm' => "Poel",
        'M' => "Poal", 'k' => "Palel", 'K' => "Pulal", 'Q' => "Qal passive",
...
fn conj_name(c: char) -> Option<&'static str> {
    Some(match c {
        'p' | 'q' => "Perfect", 'i' => "Imperfect", 'w' => "Wayyiqtol",
        'h' => "Cohortative", 'j' => "Jussive", 'v' => "Imperative",
        'r' => "Participle (act.)", 's' => "Participle (pas.)",
        'a' => "Inf. Absolute", 'c' => "Inf. Construct",
        _ => return None,
    })
}
...
fn main() -> anyhow::Result<()> {
    ...
    {
        let mut s = lex.prepare("SELECT strong, word FROM english WHERE word IS NOT NULL AND strong IS NOT NULL")?;
        for r in s.query_map([], |r| Ok((r.get::<_, i64>(0)?, r.get::<_, String>(1)?)))? {
            let (k, v) = r?; lemma.insert(k, v);
        }
    }
    let mut root_of: HashMap<i64, String> = HashMap::new();
    {
        ...
        for r in s.query_map([], |r| Ok((r.get::<_, i64>(0)?, r.get::<_, String>(1)?)))? {
            let (k, v) = r?; root_of.entry(k).or_insert(v);
        }
    }
    ...
    for path in paths {
        ...
        let mut buf = Vec::new();
        let (mut in_word, mut morph, mut lem, mut text) = (false, String::new(), String::new(), String::new());
        loop {
            match reader.read_event_into(&mut buf)? {
                Event::Start(e) if e.name().as_ref() == b"w" => {
                    in_word = true; text.clear(); morph.clear(); lem.clear();
                ...
                Event::Text(t) if in_word => {
                    let f = t.unescape()?;
                    if f.as_ref() > "z" { text.push_str(f.as_ref()); }
                }
                Event::End(e) if e.name().as_ref() == b"w" => {
                    ...
                    let strong = lem.rsplit('/').next().unwrap_or("")
    ...
    let miss: Vec<String> = con.prepare("SELECT text FROM review_missing")?
    ...
    for m in &miss {
        ...
        for (morph, strong) in readings {
            let Some(body) = morph.strip_prefix('H') else { continue };
            for seg in body.split('/') {
                let mut ch = seg.chars();
                if ch.next() != Some('V') { continue; }
                ...
                let Some(binyan) = (if allow { stem_name_full(stem) } else { stem_name(stem) }) else { continue };
                let Some(form) = conj_name(conj) else { continue };
                ...
                let root = root_of.get(strong).cloned()
        ...
        if !rows.is_empty() { n_surf += 1; out.insert(m.clone(), rows); }
    }
