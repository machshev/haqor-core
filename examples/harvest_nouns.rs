...
fn main() -> anyhow::Result<()> {
    ...
    for path in paths {
        ...
        let mut buf = Vec::new();
        let (mut iw, mut mo, mut lem, mut tx) =
            ...
            match r.read_event_into(&mut buf)? {
                ...
                Event::End(e) if e.name().as_ref() == b"w" => {
                    ...
                    if !ns.is_empty() && !mo.is_empty() {
                        gold.entry(ns).or_default().push((mo.clone(), lem.clone()));
                    }
                }
                Event::Eof => break,
                _ => {}
            ...
            if let Some((strong, is_proper)) = noun_head(mo, lem) {
                ...
                if let Some((w, g)) = lemma.get(&strong) {
                    let gloss: String = g
                        ...
                        .take(40)
                        .collect();
                    common.insert(m.clone(), (w.clone(), gloss));
                    break;
                }
            }
        }
    ...
    eprintln!(
        "// {} common-noun/adj surfaces, {} proper-noun surfaces",
        common.len(),
        proper.len()
    );
