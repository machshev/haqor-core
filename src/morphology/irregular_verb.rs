...
pub const IRREGULAR_VOCALIZATIONS: &[IrregularVerb] = &[
    ...
    // Weak-root forms whose surface the reverse parser only reconstructs under a
    // fabricated root (geminate/hollow/I-nun/I-yod cross-spellings the lexicon does
    // not attest, e.g. דממ→נדמ, יצת→נצת, תממ→תומ): after lexicon-root
    // disambiguation those readings are pruned, so list each attested surface under
    // its canonical OSHB root (Strong-sourced) for the stored/product analysis.
    IrregularVerb {
        surface: "אֹבִידָה",
        root: "אבד",
        binyan: "Hiphil",
